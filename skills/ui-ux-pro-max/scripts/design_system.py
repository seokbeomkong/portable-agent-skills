...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None,
        ...
        # Track direct query coverage separately from reasoning-enriched searches.
        # Otherwise generic fallback keywords (Minimalism/Flat Design) can make a
        # nonsense query look like a real catalog match.
        direct_matches = {
            domain: bool(search(query, domain, 1).get("results", []))
            for domain in SEARCH_CONFIG
        }
        ...
        return {
            ...
            "catalog_match": {
                "matched": any(direct_matches.values()),
                "matched_domains": [
                    domain for domain, matched in direct_matches.items() if matched
                ],
                "unmatched_domains": [
                    domain for domain, matched in direct_matches.items() if not matched
                ],
                "fallback_used": not any(direct_matches.values()),
            },
            "category": category,
            "pattern": {
...
def _catalog_fallback_notice(design_system: dict) -> str:
    match = design_system.get("catalog_match", {})
    if match.get("fallback_used"):
        return (
            "GENERAL FALLBACK: the query had no direct catalog match; "
            "broaden or translate the keywords before treating this recommendation as specific."
        )
    return ""
...
def format_ascii_box(design_system: dict) -> str:
    ...
    rendered = "\n".join(lines)
    notice = _catalog_fallback_notice(design_system)
...
def format_markdown(design_system: dict) -> str:
    ...
    notice = _catalog_fallback_notice(design_system)
    if notice:
        lines.append(f"> **General fallback:** {notice}")
        lines.append("")
...
def format_master_md(design_system: dict) -> str:
    ...
    notice = _catalog_fallback_notice(design_system)
    if notice:
        lines.append(f"**Catalog status:** General fallback — {notice}")
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        dial_parts = []
        if dials.get("variance") is not None:
